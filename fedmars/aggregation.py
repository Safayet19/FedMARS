...
def select_layers_under_budget(
    ...
    if not candidates:
        ...
        base = [max(global_credit.items(), key=lambda kv: float(kv[1]))[0]]
        base.extend(name for name in must_include if name in global_credit)
        return sorted(set(base))
    if float(budget_fraction) >= 0.999:
        chosen = set(candidates)
        chosen.update(name for name in must_include if name in global_credit)
    ...
    for name in must_include:
        if name not in global_credit or float(layer_costs.get(name, 0.0)) <= 0.0:
            continue
        if name not in selected:
            selected.append(name)
            used_cost += float(layer_costs.get(name, 0.0))
    for name in ordered:
        if name in selected:
            continue
