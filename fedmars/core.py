...
def _normalize_global_credit(raw_credit: Mapping[str, float]) -> dict[str, float]:
    ...
    return {
        name: float(np.clip((float(value) - med) / mad, -2.5, 2.5))
...
class FedMARS:
    def __init__(
        ...
        self.depth_weights = compute_depth_weights(
            self.layer_specs,
            mode=self.config.depth_weight_mode,
        )
        self.layer_costs = compute_layer_costs(self.layer_specs)
        self.layer_bits = compute_layer_bits(
            self.layer_specs,
            param_bits=self.config.param_bits,
        )
    ...
    def _sample_clients(
        ...
        choose = max(
            ...
            int(round(self.config.client_fraction * num_clients)),
        )
        ...
        for spec in self.layer_specs:
            ...
            if self.config.reference_sketch_mode == "ema_unit":
                ...
                if prev_ref is None:
                    ...
                    merged = (
                        self.config.reference_momentum * prev_ref
                        + (1.0 - self.config.reference_momentum) * unit
                    )
    ...
    def _compute_batch_layer_grads(
        self,
        model: torch.nn.Module,
        batch,
    ) -> dict[str, torch.Tensor]:
        ...
        return {
            spec.name: flatten_grads_from_model(model, spec).detach().cpu()
            for spec in self.layer_specs
        }
    ...
    def _compute_transfer_scores(
        ...
        for batch_idx in range(num_batches):
            batch = sample_batch_from_indices(
                client.dataset,
                all_indices,
                self.config.probe_batch_size,
                seed=round_seed + batch_idx,
            )
        ...
        for spec in self.layer_specs:
            ...
            for i in range(len(probe_grads)):
                for j in range(i + 1, len(probe_grads)):
                    vals.append(
                        safe_cosine(
                            probe_grads[i][spec.name],
                            probe_grads[j][spec.name],
                        )
                    )
    ...
    def _map_transfer_to_lr(self, transfer_score: float) -> float:
        return float(
            ...
            * sigmoid(
                self.config.kappa_transfer
                * (transfer_score - self.config.tau_transfer)
            )
        )
    ...
    def _probe_layer_gain(
        ...
        with torch.no_grad():
            ...
            for pname in spec.param_names:
                named_params[pname].add_(
                    -float(self.config.probe_step)
                    * step_chunks[pname].to(named_params[pname].device)
                )
    ...
    def _phase_a_client_credit(
        ...
        for group_idx, indices in enumerate(groups):
            ...
            if accum is not None:
                cluster_grads.append(
                    {name: tensor / float(repeats) for name, tensor in accum.items()}
                )

        if not cluster_grads:
            cluster_grads = [
                {spec.name: torch.zeros(spec.numel) for spec in self.layer_specs}
            ]

        probe_batch = sample_batch_from_indices(
            client.dataset,
            list(range(len(client.dataset))),
            self.config.probe_batch_size,
        ...
        for spec in self.layer_specs:
            ...
            record = compute_layer_credit(
                ...
                probe_gain=(
                    self._probe_layer_gain(global_state, spec, mixed, probe_batch)
                    if self.config.lambda_v > 0.0
                    else 0.0
                ),
    ...
    def _phase_b_client_update(
        ...
        transfer_scores = self._compute_transfer_scores(
            ...
            round_seed=(
                ...
                + 30000
                + round_idx
                + _stable_client_seed(client.client_id)
            ),
        )

        layer_lrs = {
            name: self._map_transfer_to_lr(score)
        ...
        return (
            {name: deltas[name] for name in selected if name in deltas},
            transfer_scores,
            layer_lrs,
        )
    ...
    def fit(
        ...
        for round_idx in range(self.config.num_rounds):
            ...
            ref_sketches = self._build_reference_sketches(
                global_state,
            ...
            for client in sampled_clients:
                credits, details = self._phase_a_client_credit(
                    client,
                    global_state,
                    ref_sketches,
                    round_idx,
            ...
            selection_budget = (
                1.0 if round_idx < self.config.warmup_rounds else budget_fraction
            )

            selection_report = select_layers_with_report(
                raw_global_credit,
                self.layer_bits,
                selection_budget,
                threshold,
            ...
            for spec in self.layer_specs:
                ...
                for pname, delta in aggregated_update[spec.name].items():
                    ...
                    vel.mul_(self.config.aggregation_momentum).add_(
                        (1.0 - self.config.aggregation_momentum) * delta
                    )
            ...
            comm_ratio = float(
                selection_report.selected_bits / max(selection_report.model_bits, 1)
            )

            client_to_server_bits = int(
                selection_report.selected_bits * len(sampled_clients)
            )

            server_to_client_bits = (
                int(self.model_bits * len(sampled_clients))
            ...
            round_log = {
                ...
                "selection_policy": {
                    "budget_fraction": float(selection_budget),
                    "threshold": float(threshold),
                    "mode": "fixed",
                },
                "selected_layers": selected_layers,
                "selected_layer_ratio": float(
                    len(selected_layers) / max(len(self.layer_specs), 1)
                ),
    ...
    def evaluate_clients(
        ...
        for client in clients:
            ...
            per_client[str(client.client_id)] = float(
                self.evaluate(loader)["accuracy"]
            )
        ...
        return {
            ...
            "p10_accuracy": (
                float(np.percentile(np.asarray(vals, dtype=float), 10.0))
                if vals
                else 0.0
            ),
        }
